...
docker run -p 80:8080 --env-file=env_file --name jwt-api jwt-api-test
export TOKEN=`curl -d '{"email":"icegom@gmail.com","password":"development"}' -H "Content-Type: application/json" -X POST localhost:80/auth  | jq -r '.token'`
